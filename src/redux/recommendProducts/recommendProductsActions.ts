 ...
 * 在一个thunk action中可以完成一些列连续的action操作,并且可以处理异步逻辑,业务逻辑可以从ui层面挪到这里，代码分层会更清晰
 * 
 * ThunkAction:返回一个ThunkAction的函数,void:没有返回值,RootState:传入值store的类型,unknown:扩展参数没有,RecommendProductAction:传入的action类型
 */
